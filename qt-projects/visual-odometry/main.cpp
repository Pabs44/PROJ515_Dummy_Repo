...
const string filePoints = "./test-data/points.xml";                      //Output file path for feature location within image space
...
double findCirc(Mat pose25d, vector<Matx31d> realPos3D, vector<Point2f> points, bodySections bodySection){
    ...
    case BODY_NECK:
        ...
        yMin = pose25d.at<double>(6,1)-35;
        yMax = pose25d.at<double>(6,1);
        filename = fileNeckCirc;
        break;
    case BODY_CHEST:
        xMin = pose25d.at<double>(21,0)+35; //10
        xMax = pose25d.at<double>(20,0)+5; //30
        yMin = pose25d.at<double>(3,1)-75; //75
        yMax = pose25d.at<double>(3,1)-55; //10
        filename = fileChestCirc;
        break;
    case BODY_WAIST:
        xMin = pose25d.at<double>(2,0)-45; //10
        xMax = pose25d.at<double>(1,0)+20; //30
        yMin = pose25d.at<double>(3,1)+80;
        yMax = pose25d.at<double>(3,1)+95;
        filename = fileWaistCirc;
        break;
    case BODY_HIP:
        xMin = pose25d.at<double>(21,0)+5; //10 //pose25d.at<double>(3,0) = hipL
        xMax = pose25d.at<double>(20,0)+15; //50 //pose25d.at<double>(2,0) = hipR
        yMin = pose25d.at<double>(0,1);
        yMax = pose25d.at<double>(0,1)+45;
    ...
    if(!fsCirc.isOpened()){
        cout << "Could not open circumference point file" << filename <<", please try another path" << endl;
        return -1;
    }
    fsCirc << "points" << "{";
...
void featureTracking(Mat img_1, Mat img_2, vector<Point2f>& points1, vector<Point2f>& points2, vector<uchar>& status){
    ...
    vector<float> err;
    Size winSize = Size(101,101); //101,101
    cout << "Tracking: Finding term criteria" << endl;
    TermCriteria termcrit = TermCriteria(TermCriteria::COUNT+TermCriteria::EPS, 30, 0.001); //50, 0.001
...
//        while(waitKey(10) != 'x') imshow("Image", hImgRect);
        ...
        for(int j = 0; j < (int)points[currentFrame-1].size(); j++){
            ...
            //HEAD
            if(((int)points[currentFrame-1].at(j).x>=345 && (int)points[currentFrame-1].at(j).x<=500)
                    && ((int)points[currentFrame-1].at(j).y>=180 && (int)points[currentFrame-1].at(j).y<=270))
            {
            ...
            //FEET
            else if(((int)points[currentFrame-1].at(j).x>=370 && (int)points[currentFrame-1].at(j).x<=585)
                    && ((int)points[currentFrame-1].at(j).y>=1595 && (int)points[currentFrame-1].at(j).y<=1620))
            {
                circle(hImgRect3, points[currentFrame-1].at(j), 5, Scalar(255,0,0), -1);
                circle(hImgRect3, points[currentFrame].at(j)+Point2f(imgSize.width, 0), 5, Scalar(255,0,0), -1);
        ...
        if(currentFrame == 1){
            int pointNum = 46;      //HEAD
            int pointNumP2 = 4440;  //FEET
        ...
        if(!fs3D.isOpened()){
            cout << "Could not open 3D output file, try another path" << endl;
            return -1;
        }
        ...
        for (int j = 0; j < (int)points[currentFrame-1].size()-1; j++){
            ...
            if(realPos3Dtmp(2) < double(1900.00) || realPos3Dtmp(2) > double(2090.00)){
                points[currentFrame-1].erase(points[currentFrame-1].begin() + j - indexCorrection);
                points[currentFrame].erase(points[currentFrame].begin() + j - indexCorrection);
                indexCorrection++;
            }else{
        ...
        fs3D.open(fileCircs, FileStorage::WRITE);
        if(!fs3D.isOpened()){
            cout << "Could not open circumference output file, try another path" << endl;
            return -1;
        }
        ...
        for(int bodySection = BODY_NECK; bodySection <= BODY_HIP; bodySection++){
            ...
            cout << chosenSection << " circumference: " << circVal << " mm / " << circVal/10 << " cm" << endl;
